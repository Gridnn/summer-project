...
from normalize import clean_wiki, clean_punct, clean_tag, clean_no_chinese
...
def punct_diff(dico: Dict) -> Dict[str, str]:
    ...
    if args.lang.lower() == 'zh':
      rev['after'] = [clean_no_chinese(sent) for sent in dico['after']]
      rev['before'] = [clean_no_chinese(sent) for sent in dico['before']]
      rev['rem'] = [clean_no_chinese(sent) for sent in dico['rem']]
      rev['add'] = [clean_no_chinese(sent) for sent in dico['add']]
    return rev
    return rev
